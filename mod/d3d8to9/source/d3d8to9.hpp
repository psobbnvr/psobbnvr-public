...
public:
	...
	// psobbvr: remember the (converted + overridden) presentation parameters
	// so a lost device can be reset internally without the game's help.
	void RememberPresentParameters(const D3DPRESENT_PARAMETERS &PresentParams);

private:
	void ApplyClipPlanes();
	void ReleaseShadersAndStateBlocks();

	// psobbvr: PSO self-terminates ("display properties changed" dialog) the
	// moment Present hands it D3DERR_DEVICELOST - recover here instead.
	HRESULT HandlePresentResult(HRESULT hr);
	...
	// psobbvr: state for the internal lost-device recovery (HandlePresentResult).
	D3DPRESENT_PARAMETERS LastPresentParams = {};
	bool HaveLastPresentParams = false;
	int DeviceLossFrames = 0;
};
...
private:
	...
	// psobbvr: contiguous tightly-packed mip-chain buffer handed out by
	// LockRect on the D3D9Ex-remapped (DEFAULT+DYNAMIC) path, emulating the
	// D3D8 MANAGED memory layout - see LockRect in d3d8to9_texture.cpp.
	std::vector<uint8_t> ShadowChain;
	UINT ShadowLevels = 0;
